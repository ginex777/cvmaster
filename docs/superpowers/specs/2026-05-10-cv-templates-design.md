# CV Template System â€” Design Spec
...
Users can assign one of three visual PDF templates to each MasterCv. The template determines the layout when a CV PDF is generated for download. The choice persists on the MasterCv and is selectable inline in the CV library â€” no modal required.
...
| Question | Decision |
|---|---|
| Where is template stored? | On `MasterCv` â€” one template per source CV |
...
| Save mechanism | Optimistic update â€” PATCH fires immediately on click, rolls back on error |
...
backend/src/pdf/
  renderers/
    renderer.interface.ts      â† CvLayout type + CvRenderer interface
    pdf-utils.ts               â† shared: wordWrap, pageBreak, sanitize (extracted from pdf.service.ts)
    classic.renderer.ts        â† single-column serif layout
    modern.renderer.ts         â† two-column sidebar layout
    editorial.renderer.ts      â† dark header, pill tags
  pdf.service.ts               â† updated: dispatcher only, delegates to renderers
```
...
### `pdf-utils.ts` â€” Shared Utilities

Extracted from current `pdf.service.ts`:
- `sanitizeText(text: string): string`
- `wrapText(text: string, maxChars: number): string[]`
- `addPageIfNeeded(page, y, lineHeight, doc): { page, y }` â€” returns a new page if `y` would overflow
...
- Bullet points with `Â·` prefix, 12pt indent
...
- Main column: section labels in 9pt bold accent-colored, job title 9pt bold, job meta 7.5pt gray, bullets with `â–¸` prefix
- Page break: if sidebar overflows, extend sidebar fill rectangle onto new page
- Default template â€” used for new MasterCvs unless changed
...
- Bullets with `â€”` prefix in light gray

### `pdf.service.ts` â€” Updated Dispatcher
...
#### `PATCH /cvs/:id` â€” Add `template` to Zod schema
...
Both fields remain optional â€” a request can update name only, template only, or both.

#### `GET /applications/:id/pdf` â€” Read template from MasterCv
...
Same for the bundle ZIP endpoint â€” reads `masterCv.template` before generating the CV PDF inside the archive.

### Error Handling

- Unknown `template` value from DB â†’ `default` branch in switch â†’ falls back to `ModernRenderer`, never throws
- `render()` failure â†’ caught in `ApplicationsService`, re-thrown as `InternalServerErrorException('PDF-Generierung fehlgeschlagen')`
...
**New signal per CV for optimistic update â€” inline tracking via the existing `cvs` signal array.**
...
**Template â€” add picker below each CV card's name row:**
...
| `CvTemplatePickerComponent` | Dumb | New â€” 3-option inline picker |
...
| `GET` | `/applications/:id/pdf` | JWT + Owns | â€” | `200 application/pdf` (uses `masterCv.template`) |
...
Superseded by `docs/final-application-audit-2026-05-13.md`. This spec is retained as design context only; it is no longer an active task checklist.
